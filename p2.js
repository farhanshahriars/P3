...
function playAudio() {
  ...
  var audio = new Audio();
  audio.src = audioList[Math.floor(Math.random() * 9)];
  audio.play();
}
...
function loadStartQuestion() {
  var loadImage = imageList[Math.floor(Math.random() * 9)];
...
/*
    ...
    also calls the checkAnswer function to determine the users correctness.
